        ...
        <table style="width:100%;">
            <thead>
                <tr style="background-color:#2d4154;color:#fafffe;">
                    ...
                    <th style="width: 28%; padding-left: 10px;">Nomor Kas Masuk</th>
            ...
            <tbody>
                ...
                @endphp
                @foreach ($data_kas_masuk as $data)
                    <tr style="background-color:#f3f3f3;">
                        <td style="text-align: center; border-right: 0.5px solid;">{{ ++$no }}</td>
                        <td style="padding-left: 10px;">{{ $data->nomor_kas_masuk }}</td>
                        ...
                        <td>Rp. {{ number_format($data->nominal_pemasukan, 2, ',', '.') }}</td>
                        @php $total_nominal_pemasukan += $data->nominal_pemasukan; @endphp
                    </tr>
                @endforeach
                <tr style="background-color:#ffffff;">
                    <td colspan="4" style="padding-top: 10px;"></td>
                    <td style="text-align: right; padding-right: 15px; padding-top: 10px;"><b>Total</b></td>
                    <td style="padding-top: 10px;"><b>Rp. {{ number_format($total_nominal_pemasukan, 2, ',', '.') }}</b></td>
                </tr>
            </tbody>
        </table>
    </main>
